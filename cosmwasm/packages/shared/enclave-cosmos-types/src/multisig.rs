...
use crate::types::{CosmosAddress, CosmosPubKey};

use enclave_crypto::traits::VerifyingKey;

use cosmwasm_proto::proto::crypto::multisig::multisig::MultiSignature;

use super::traits::CosmosAminoPubkey;
use crate::errors::{EncodingError, SignatureError};
...
impl CosmosAminoPubkey for MultisigThresholdPubKey {
    fn get_address(&self) -> CosmosAddress {
        ...
        CosmosAddress(address_bytes.to_vec())
    }
...
fn decode_multisig_signature(raw_blob: &[u8]) -> Result<Vec<Vec<u8>>, SignatureError> {
...
#[allow(unused)]
fn decode_multisig_signature_old(raw_blob: &[u8]) -> Result<Vec<Vec<u8>>, SignatureError> {
    trace!("decoding blob: {:?}", raw_blob);
    let blob_size = raw_blob.len();
    if blob_size < 8 {
        warn!("Multisig signature too short. decoding failed!");
        return Err(SignatureError::ParsingError);
    }
